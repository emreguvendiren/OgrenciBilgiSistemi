...
{
    public partial class VizeDurum : Form
    {
        MySqlConnection con;
        MySqlCommand cmd;
        MySqlDataAdapter dr;
        ...
        private void VizeDurum_Load(object sender, EventArgs e)
        {
            ...
            cmd.Connection = con;
            cmd.CommandText = "SELECT ogrenci.ogrNo,ogrenci.isim,ogrenci.SoyIsim,ders.DersIsim,vize.VizeNotu FROM vize INNER JOIN ders ON vize.dersNo = ders.DersNo " +
                "INNER JOIN ogrenci ON vize.ogrNo = ogrenci.OgrNo WHERE ogrenci.ogrNo = '" + ogrno + "'";
