...
{
    public partial class VizeOrtalamaGoruntule : Form
    {
        MySqlConnection con;
        MySqlCommand cmd;
        MySqlDataAdapter dx;
        ...
        public VizeOrtalamaGoruntule()
        {
            InitializeComponent();
        }

        private void VizeOrtalamaGoruntule_Load(object sender, EventArgs e)
        {
            ...
            foreach (var item in alinanDersler)
            {
                ...
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = "SELECT ders.DersNo,ders.DersIsim,avg(vizenotu) as VizeNotu FROM vize INNER JOIN ders on ders.DersNo = vize.dersNo WHERE ders.DersNo = '" + item + "' group by dersIsim";
